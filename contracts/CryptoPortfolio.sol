...
contract CryptoPortfolio is AccessControl {
    ...
    event CryptoPortfolioNewPlayer(
        address player,
        bool[] isLong,
    ...
    event CryptoPortfolioStarted(int192 startingPrice, bytes32 gameId);
    //нужно ли сюда собирать финальные результаты по типу [up,down,down,up];
    event CryptoPortfolioFinalized(int192[] finalPrices, bytes32 gameId);
    event CryptoPortfolioCancelled(bytes32 gameId);
    ...
    mapping(address => bool[]) public playerPredictions;
     ...
     */
    function play(bool[] memory predictions, uint256 depositAmount) public {
    ...
    function setStartingPrice(
        ...
        emit CryptoPortfolioStarted(priceData, currentGameId);
    }
    ...
    function finalizeGame(
        ...
        if (players.length < 2) {
            ...
            delete players;
            emit CryptoPortfolioCancelled(currentGameId);
            packedData = 0;
            startingPrice = 0;
        ...
        for (uint i = 0; i < players.length; i++) {
            ...
            for (uint j = 0; j < unverifiedReports.length; j++) {
                if (j == 0) {
                    if (
                        uint192(finalPrices[0]) > startingPrice &&
        ...
        emit CryptoPortfolioFinalized(finalPrices, currentGameId);
